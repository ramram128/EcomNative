...
/* =================== PRODUCT CARD (EXACT LOOK RESTORED) =================== */
const ProductCard = ({ product, onPress, numColumns }: { product: Product; onPress: () => void; numColumns: number }) => {
  ...
  const priceData = useMemo(() => {
    ...
    const isDiscounted = !isNaN(regNum) && regNum > saleNum;

    return {
      showBoth: isDiscounted,
      discount: isDiscounted ? Math.round(((regNum - saleNum) / regNum) * 100) : 0,
      inrSale: formatINR(sale),
      inrRegular: formatINR(reg),
      isVar: product.type === 'variable'
    };
  }, [product]);
      ...
      <View style={styles.cardInfo}>
        <View style={styles.nameRow}>
          ...
          {priceData.showBoth && (
            <View style={styles.inlineDiscountBadge}>
              <Text style={[styles.inlineDiscountText, { fontSize: 10 * scale }]}>-{priceData.discount}%</Text>
            </View>
          )}
        </View>
        ...
        <View style={styles.cardPriceRow}>
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            {priceData.isVar && <Text style={{ fontSize: 10 * scale, color: '#888' }}>From </Text>}
            <Text style={[styles.cardPrice, { fontSize: 17 * scale }]}>₹{priceData.inrSale}</Text>
          </View>
        </View>

        {priceData.showBoth && (
          <Text style={styles.strikePrice}>₹{priceData.inrRegular}</Text>
        )}
      </View>
    </TouchableOpacity>
  );
...
/* =================== MAIN LAYOUT =================== */
export default function HomeCrystal({ products, onPress, refreshing, onRefresh, searchQuery, setSearchQuery }: HomeLayoutProps) {
  ...
  const renderItem = useCallback(({ item }: { item: SectionItem }) => {
    ...
    const hWidth = (width - 32) / (item.columns + 0.2);
      ...
      <View style={styles.sectionWrapper}>
        ...
        {item.mode === 'scroll' ? (
          <FlatList
            ...
            renderItem={({ item: p }) => (
              <View style={{ width: hWidth, marginRight: 15 }}>
                <ProductCard product={p} onPress={() => onPress(p)} numColumns={item.columns} />
              </View>
            )}
          />
        ) : (
          <View style={styles.productGrid}>
            {item.data.map((p, idx) => (
              <View key={p.id || idx} style={{ width: `${100 / item.columns}%`, padding: 8 }}>
                <ProductCard product={p} onPress={() => onPress(p)} numColumns={item.columns} />
              </View>
            ))}
          </View>
...
const styles = StyleSheet.create({
  brandName: { fontSize: 26, fontWeight: '900', color: JOY_COLORS.primary, paddingHorizontal: 20, marginBottom: 6 },
  sectionWrapper: { marginBottom: 20 },
  sectionHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 20, marginBottom: 12 },
  sectionTitle: { fontSize: 24, fontWeight: '900', color: JOY_COLORS.text },
  ...
  bannerBadge: { color: '#fff', fontSize: 10, fontWeight: '800', marginBottom: 6 },
  ...
  inlineDiscountBadge: { backgroundColor: '#e6f4ea', borderRadius: 6, paddingHorizontal: 6, paddingVertical: 2 },
  ...
  strikePrice: { fontSize: 11, color: '#999', textDecorationLine: 'line-through', marginTop: 2 }
});